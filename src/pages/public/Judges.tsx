...
export default function Judges() {
  const judges = [
    { name: "CHARLES BRIGGS", title: "Art Director" },
    { name: "KELVIN JAMES", title: "Educator" },
    { name: "CLINTON LUKAS", title: "Creative Director" },
    { name: "DENNIS FRANK", title: "Judge" },
    { name: "SAMPSON COURT", title: "Art Critic" },
    { name: "LADY DEBORAH", title: "Panel Lead" },
    { name: "LIVING STONE", title: "Judge" },
    { name: "JONES KIT", title: "Evaluator" }
  ];
          ...
          {judges.map((judge, i) => (
             <div key={i} className="bg-white border text-center border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-lg transition-all">
                <div className="w-32 h-32 mx-auto rounded-full bg-slate-100 overflow-hidden mb-4">
                  <img src={`https://i.pravatar.cc/150?img=${i+10}`} alt={judge.name} className="w-full h-full object-cover" />
                </div>
                <h3 className="text-lg font-bold">{judge.name}</h3>
                <p className="text-sm text-slate-500 uppercase tracking-widest mt-1">{judge.title}</p>
             </div>
          ))}
        </div>
      </section>
